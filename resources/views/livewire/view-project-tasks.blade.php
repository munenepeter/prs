<div class="uk-width-1-1">
	...
	<div class="uk-flex uk-flex-between uk-flex-middle">
		...
		<a href="{{ route('projects.index') }}"
		   class="uk-button-primary uk-button">All Projects</a>
	</div>
					...
					<tr>
						...
						<td>
							<div class="uk-button-group">
								<button uk-toggle="target: #edit-task-{{ $task->id }}"
										type="button"
										class="uk-button uk-button-secondary uk-margin-small-right"
								>Edit
								</button>
								<livewire:edit-task-modal key="{{ $task->id . ' ' . $task->name }}"
														  :task_id="$task->id"
								/>

								<button type="button" onclick="confirm('Are you sure you want to delete this project task?') || event.stopImmediatePropagation()"
